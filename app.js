...
function loadUserProfile() {
  const userName = localStorage.getItem('gigshield_user');
  ...
  const userEmail = localStorage.getItem('gigshield_email');
  ...
  if (userName) {
    ...
    if (homeNameEl) homeNameEl.textContent = userName;

    const homeSubEl = document.getElementById('homeUserSub');
    if (homeSubEl) homeSubEl.textContent = (userEmail || '') + ' · Bangalore';
    ...
    if (profileSub) profileSub.textContent = (userEmail || '') + ' · Bangalore, KA';
  }
}
...
function handleSignOut() {
  ...
  localStorage.removeItem('gigshield_email');
...
function showToast(msg) {
  ...
  const bg = isLight ? '#6C5CE7' : '#CBFF00';
  const fg = isLight ? '#FFFFFF' : '#0D0D0D';
  t.style.cssText = `position:fixed;top:72px;left:50%;transform:translateX(-50%);
    ...
    font-family:'Inter',sans-serif;background:${bg};color:${fg};max-width:85%;
