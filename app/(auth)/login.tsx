...
import { View, Text, TouchableOpacity, StyleSheet, ActivityIndicator, KeyboardAvoidingView, Platform, TextInput, Alert } from 'react-native';
...
export default function LoginScreen() {
  ...
  async function handleQuickDemoLogin() {
    setLoading(true);
    // Populate gigshield local storage contexts
  ...
  async function handleCustomLogin() {
    setLoading(true);
    const finalName = username.trim() || 'Hackathon Judge';
    await AsyncStorage.setItem('gigshield_user', finalName);
    await AsyncStorage.setItem('gigshield_email', `${finalName.toLowerCase().replace(' ', '')}@gigshield.ai`);
    await AsyncStorage.setItem('gigshield_city', 'mumbai');
    await AsyncStorage.setItem('gigshield_plan', 'pro');
    
    const demoUser = {
      id: 'custom_' + Date.now(),
      email: `${finalName.toLowerCase().replace(' ', '')}@gigshield.ai`,
      name: finalName,
      provider: 'email' as const,
      createdAt: new Date().toISOString(),
    };
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={styles.card}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Enter your personal data to access your account</Text>
        </View>
        ...
        <TextInput
          style={styles.input}
          placeholder="Display Name (e.g. Judge)"
        ...
        <TouchableOpacity 
          style={[styles.primaryAuthBtn, { backgroundColor: Colors.dark.lime, marginTop: 4 }]} 
          onPress={handleCustomLogin}
        >
          <Text style={[styles.primaryAuthText, { color: Colors.dark.background }]}>Sign in</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  ambientGlow: {
    ...
    height: 300,
    borderRadius: 200,
    backgroundColor: Colors.dark.limeDark,
    opacity: 0.15,
